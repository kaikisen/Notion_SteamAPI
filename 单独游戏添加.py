...
def get_appid_from_input(user_input):
    # 如果输入的是纯数字
    ...
    raise ValueError("无法识别 AppID，请输入数字或 Steam 商店链接")


add_input = input("请输入appid或Steam链接:")

add_appid = get_appid_from_input(add_input)
...
def get_steam_game_info(appid):
    ...
    # 获取真实封面
    try:
        header_img = soup.select_one("#gameHeaderImageCtn > img")

        if header_img and header_img.get("src"):
            cover = header_img["src"]
        else:
...
def create_notion_page(game):
    ...
    data = {
        ...
        "properties": {
            ...
            "上次游玩时间": {"date": {"start": datetime.datetime.now().strftime("%Y-%m-%d")}}
        }
    }
